...
-- This script adds the new tables for:
-- 1. Webhook System
...
-- Add webhook relationship to users table (already exists, just documenting)
-- ALTER TABLE users ADD COLUMN IF NOT EXISTS webhooks_created INTEGER DEFAULT 0;

-- =====================================================
-- 2. EMAIL MARKETING TABLES (Optional - using external services)
-- =====================================================
...
-- Insert sample email templates (only if admin user exists)
DO $$ 
DECLARE
    admin_user_id UUID;
BEGIN
    ...
    IF admin_user_id IS NOT NULL THEN
        ...
        INSERT INTO email_templates (name, type, subject, html_content, variables, created_by)
        SELECT 
            'Price Alert Template',
            'price_alert',
            '🔥 Price Drop Alert: {{product_name}} is now {{new_price}} {{currency}}!',
            ...
            ARRAY['product_name', 'product_url', 'old_price', 'new_price', 'savings', 'savings_percent', 'currency'],
            admin_user_id
        WHERE NOT EXISTS (SELECT 1 FROM email_templates WHERE type = 'price_alert');

        -- Insert sample social media templates
        INSERT INTO social_media_templates (name, type, content, hashtags, variables, platforms, created_by)
        SELECT 
            'Product Launch Template',
            'product_launch',
            '🚀 New Product Alert! Check out {{product_name}} - now available for just {{price}} {{currency}}! {{product_url}}',
            ARRAY['#NewProduct', '#Deal', '#Shopping'],
            ARRAY['product_name', 'price', 'currency', 'product_url'],
            ARRAY['twitter', 'facebook', 'linkedin']::social_media_platform[],
            admin_user_id
        WHERE NOT EXISTS (SELECT 1 FROM social_media_templates WHERE type = 'product_launch');

        -- Insert sample feature flags
        INSERT INTO feature_flags (name, key, description, type, value, default_value, created_by)
        SELECT 
            'Advanced Analytics Dashboard',
            'advanced_analytics',
            'Enable advanced analytics dashboard with AI insights',
            'boolean',
            'true'::jsonb,
            'false'::jsonb,
            admin_user_id
        ...
        RAISE NOTICE 'Sample data inserted successfully with admin user: %', admin_user_id;
    ELSE
        RAISE NOTICE 'No admin user found - skipping sample data insertion';
        RAISE NOTICE 'You can add sample data later after creating an admin user';
    END IF;
END $$;
